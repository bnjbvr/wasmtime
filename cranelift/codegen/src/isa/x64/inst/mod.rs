...
pub enum Inst {
    ...
    /// Sign-extended loads: movs (bl bq wl wq lq) addr reg.
    MovSX_RM_R {
        ext_mode: ExtMode,
        src: RegMem,
        dst: Writable<Reg>,
    ...
    /// Integer comparisons: cmp (b w l q) (reg addr imm) reg.
    Cmp_RMI_R {
        size: u8, // 1, 2, 4 or 8
        src: RegMemImm,
        dst: Reg,
    ...
    /// Float arithmetic/bit-twiddling: (add sub and or xor mul adc? sbb?) (32 64) (reg addr imm) reg
    XMM_RM_R {
        op: SseOpcode,
        src: RegMem,
        dst: Writable<Reg>,
    ...
    /// Float instructions: mov (64 32) reg reg
    XMM_R_R {
        op: SseOpcode,
        src: Reg,
        dst: Writable<Reg>,
