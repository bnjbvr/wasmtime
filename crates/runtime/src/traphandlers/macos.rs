...
unsafe fn handle_exception(request: &mut ExceptionRequest) -> bool {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_arch = "x86_64")] {
            ...
            extern "C" {
                /// A small stub we're going to start unwinding the wasm call frames.
                /// See below for an explanation of why it's needed.
            ...
            // We can't just redirect within the `unwind` function, because we'd like to report the
            // call frame at the faulting PC, while keeping the previous LR around to be able to
            // unwind other frames.
            //
            // This must be kept in sync with macos_aarch64.S, which is built only on mac aarch64.
            let resume = |state: &mut ThreadState, pc: usize, jmp_buf: usize| {
                // Create a fake frame by manually subtracting to SP.
                //
                // SP is always 16-bytes in aarch64's code: either we're in the prologue before
                // allocating any stack, and SP is 16-bytes aligned; or we did just allocated and
                // maintained the 16-bytes alignment. Remember, on aarch64, the return address is
                // not pushed onto the stack by the "blr" instruction (it lives in LR instead).
                state.__sp -= 16;

                // Fill the frame with both PC and LR, so both the current frame and caller frame
                // appear in the backtrace during unwinding.
                *((state.__sp + 8) as *mut u64) = pc as u64;
                *((state.__sp + 0) as *mut u64) = state.__lr;

                state.__fp = state.__sp + 16;

                // Fill in the 2 arguments to unwind here, as if we were calling unwind: this
                // avoids arguments shuffling in the `wasmtime_macos_aarch64_unwind` assembly stub.
                // Pass the unwind function pointer in the next available argument register.
